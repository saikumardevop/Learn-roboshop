...
func_schema_setup(){
 if [ "$ schema_setup" == "mongo" ] ; then
   ...
   mongo --host mongodb.saikumar22.store </app/schema/${component} &>>$log_file
   func_stat_check $?
 fi
 if [ "${schema_setup}" == "mysql" ]; then
   ...
   func_print_head  "Load Schema"
   mysql -h mysql.saikumar22.store  -uroot -p$mysql_root_password < /app/schema/${component}.sql &>>$log_file
   func_stat_check $?
 fi
